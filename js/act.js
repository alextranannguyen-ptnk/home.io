...
document.addEventListener('DOMContentLoaded', () => {
  const slides = document.querySelector('.school-slides');
  const images = document.querySelectorAll('.school-img');
  const total = images.length / 2; // since you duplicated images
  const imageWidth = images[0].clientWidth + parseFloat(getComputedStyle(images[0]).marginLeft) * 2;
  ...
  function moveSlide() {
    ...
    slides.addEventListener('transitionend', () => {
      ...
      // if we’ve reached the end of the first set, instantly jump back to start
      if (index >= total) {
  ...
  setInterval(moveSlide, 1600); // move every 1.6 seconds
});
